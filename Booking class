...
class Booking:
    ...
    def is_active(self):
        ### This checks if booking is currently active (Reserved or CheckedIn)
        return self.status in [self.STATUS_RESERVED, self.STATUS_CHECKED_IN]

 def get_duration(self):
        ### This calculates number of nights for the booking
        try:
            start = datetime.strptime(self.start_date, "%Y-%m-%d")
            end = datetime.strptime(self.end_date, "%Y-%m-%d")
    ...
    def to_dict(self):
        ### Convert booking details to dictionary for saving to file
        return {
