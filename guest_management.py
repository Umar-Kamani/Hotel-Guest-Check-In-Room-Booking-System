...
def modify_guest():
    ...
    for k in old_guest_filter:
        if k.guest_id == guest_id_modify:
            ...
            if details_modify == '1': #take this out
                edit_guest_id(guest_id_modify)
...
def edit_guest_passport_number(guest_passport_number_modify): #remove modify of guest ID and make this function into modify passport/id number
    while True:
        new_guest_passport_number_id = input("Please enter the new guest Passport: ")
        if new_guest_passport_number_id == guest_passport_number_modify:
